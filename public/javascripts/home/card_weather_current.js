const weatherInterval = 60000 * 10 // n mins
...
async function processWeather (xmlWeather) {
  const observations = xmlWeather.getElementsByTagName('observations')
  const observationsTime = observations[0].getAttribute('time')
  ...
  for (s of stations) {
    if (s.getAttribute('name') === 'Dublin') {
      ...
      const symbolName = await getSymbolName(getStringForAttribute(s, 'weather_text'), observationsTime)
      // console.log(symbolName)
      d3.select('#hero-weather__symbol')
        .html('<img src = "/images/icons/weather/Met50v2/' + symbolName + '.png" alt = "weather symbol">')
...
async function getSymbolName (text, time) {
  const symbolLookup = await d3.csv('/data/weather/plain-language-symbol-lookup.csv')
  const symbolNo = symbolLookup.filter((r) => {
  ...
  const isDay = await isDaytime(time)
...
async function isDaytime (t) {
  const d = new Date(t)
  const data = await d3.csv('/data/weather/ireland-sunrise-sunset-by-month.csv')

  // oging to approximate to the floor hour
  const sunriseHour = parseInt(data[d.getMonth()].sunrise)
  const sunsetHour = parseInt(data[d.getMonth()].sunset)
